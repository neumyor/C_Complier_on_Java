...
public class BeqExp extends IMExp {
    protected BeqExp(IMItem item1, IMItem item2, IMItem item3) {
    ...
    public void toCode(RegPool pool) {
        ...
        if (a instanceof ConstValueEntry && b instanceof ConstValueEntry) {
            ...
            Integer aValue = (Integer) ((ConstValueEntry) a).getValue();
            ret.addLine(String.format("seq %s, %s, %d", pool.findNoLoad(toAssign), bReg, aValue));
        } else if (b instanceof ConstValueEntry) {
            String aReg = pool.find(a);
            Integer bValue = (Integer) ((ConstValueEntry) b).getValue();
            ret.addLine(String.format("seq %s, %s, %d", pool.findNoLoad(toAssign), aReg, bValue));
        } else {
            String aReg = pool.find(a);
            String bReg = pool.find(b);
            ret.addLine(String.format("seq %s, %s, %s", pool.findNoLoad(toAssign), aReg, bReg));
        }
